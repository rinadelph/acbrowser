...
```bash
acbrowser dashboard install
acbrowser dashboard start
acbrowser open example.com    # session appears in dashboard
...
```bash
acbrowser network requests --type xhr,fetch --method POST
acbrowser network request <requestId>
```
...
```bash
echo '[["open","example.com"],["snapshot"]]' | acbrowser batch --json
```
...
acbrowser network har stop ./trace.har
```
...
acbrowser is now 100% native Rust. The Node.js/Playwright daemon has been completely removed -- no Node.js runtime or Playwright dependency is required to run the daemon. The Rust native daemon is now the only implementation.
...
```bash
npm install -g acbrowser      # 7 MB install
acbrowser install              # download Chrome
...
acbrowser --provider browserless open example.com
acbrowser --provider browserless screenshot ./page.png
```
...
```bash
acbrowser screenshot --screenshot-dir ./shots
...
```bash
# Via flag
acbrowser --native open example.com
...
- The native and Node.js daemons share the same session socket. Use `acbrowser close` before switching between modes.
...
- **`--selector` flag for scroll** -- Scroll within a specific container element instead of the page: `acbrowser scroll down 500 --selector "div.scroll-container"`
...
# Security flags
acbrowser --content-boundaries --allowed-domains "example.com,*.example.com" --max-output 50000 open https://example.com

# Download path
acbrowser --download-path ./downloads open https://example.com

# Scroll within container
acbrowser scroll down 500 --selector "div.content"
```
...
acbrowser keyboard inserttext "pasted text"
...
- **Configuration file support** -- Automatic loading from user (`~/.acbrowser/config.json`) and project (`./acbrowser.json`) directories with priority-based merging.
...
```bash
# Via -p flag
acbrowser -p kernel open https://example.com
...
```bash
acbrowser --ignore-https-errors open https://localhost:3000
```

- **Enhanced cookie management** - Extended `cookies set` command with additional flags for setting cookies before page load

```bash
acbrowser cookies set session_id "abc123" --url https://app.example.com --httpOnly --secure
acbrowser cookies set token "xyz" --domain .example.com --path /api --expires 1735689600
```
...
acbrowser -p browseruse open https://example.com
...
```bash
acbrowser --profile ~/.myapp-profile open myapp.com
...
```bash
acbrowser --cdp "wss://browser-service.com/cdp?token=..." snapshot
```

- **`download` command** - Trigger downloads and wait for completion

```bash
acbrowser download @e1 ./file.pdf
acbrowser wait --download ./output.zip --timeout 30000
```

- **Browser launch configuration** - Fine-grained control over browser startup

```bash
acbrowser --args "--disable-gpu,--no-sandbox" open example.com
acbrowser --user-agent "Custom UA" open example.com
acbrowser --proxy-bypass "localhost,*.internal" open example.com
```
...
acbrowser snapshot  # No --cdp needed after connect
```

- **`--proxy` flag** - Configure browser proxy with optional authentication

```bash
acbrowser --proxy http://user:pass@proxy.com:8080 open example.com
```
