...
acbrowser includes security features to protect against credential exposure, prompt injection via untrusted page content, and unauthorized browser actions.

All security features are opt-in. By default, acbrowser imposes no restrictions on navigation, actions, or output. Enable these features as needed for your deployment -- existing workflows are unaffected until you explicitly activate a feature.
...
- **Domain filter timing on remote connections.** When connecting to a pre-existing browser via CDP or a cloud provider, pages may have already loaded content before the domain filter is installed. acbrowser navigates disallowed pages to `about:blank` after the filter is active, but resources loaded before that point are not retroactively blocked.
...
Profiles are stored in `~/.acbrowser/auth/` and always encrypted with AES-256-GCM. If `AGENT_BROWSER_ENCRYPTION_KEY` is not set, a key is auto-generated at `~/.acbrowser/.encryption-key` on first use. Back up this file or set the environment variable explicitly for portability.
...
```bash
acbrowser --content-boundaries snapshot
# or
...
```bash
acbrowser --allowed-domains "example.com,*.example.com,github.com" open https://example.com
# or
...
```bash
acbrowser --action-policy ./policy.json open https://example.com
# or
...
```bash
acbrowser --confirm-actions eval,download --confirm-interactive eval "document.title"
# Prompts: Allow? [y/N]
```
...
```bash
acbrowser --max-output 50000 get text body
# or
