...
Run acbrowser from a Next.js app on Vercel using Vercel Sandbox.
A Linux microVM spins up on demand, runs acbrowser + Chrome, and
...
step before acbrowser can launch Chrome. Use a sandbox snapshot
...
export async function snapshotUrl(url: string) {
  return withBrowser(async (sandbox) => {
    ...
    const result = await sandbox.runCommand("acbrowser", [
      "snapshot", "-i", "-c",
    ]);
...
acbrowser + Chromium from scratch (~30 seconds). A **sandbox snapshot**
...
This is different from an acbrowser *accessibility snapshot* (which
...
acbrowser + Chromium, saves the VM state, and prints the snapshot ID:
...
export async function GET() {
  const result = await withBrowser(async (sandbox) => {
    await sandbox.runCommand("acbrowser", [
      "open", "https://example.com/pricing",
    ]);
    const snap = await sandbox.runCommand("acbrowser", [
      "snapshot", "-i", "-c",
    ]);
    await sandbox.runCommand("acbrowser", ["close"]);
...
[`examples/environments/`](https://github.com/vercel-labs/acbrowser/tree/main/examples/environments).
