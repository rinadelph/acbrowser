...
AGENT_BROWSER_SESSION=agent1 acbrowser click "#btn"
...
The simplest way to reuse your existing login state: pass a Chrome profile name to `--profile`. acbrowser copies the profile to a temp directory (read-only snapshot) and launches Chrome with your existing cookies and sessions.
...
acbrowser --profile "Work" open https://app.example.com

# Or via environment variable
AGENT_BROWSER_PROFILE=Default acbrowser open https://gmail.com
```
...
acbrowser --profile ~/.myapp-profile open myapp.com/dashboard

# Or via environment variable
AGENT_BROWSER_PROFILE=~/.myapp-profile acbrowser open myapp.com
```
...
If you are already logged in to a site in Chrome, you can grab that auth state and reuse it in acbrowser. This is the fastest way to bypass login flows, OAuth, SSO, or 2FA.
...
acbrowser --session-name twitter open twitter.com

# Login once, then state persists automatically
acbrowser --session-name twitter click "#login"

# Or via environment variable
export AGENT_BROWSER_SESSION_NAME=twitter
acbrowser open twitter.com
```

State files are stored in `~/.acbrowser/sessions/` and automatically loaded on daemon start.
...
```bash
# Valid session names
acbrowser --session-name my-project open example.com
acbrowser --session-name test_session_v2 open example.com

# Invalid (will be rejected)
acbrowser --session-name "../bad" open example.com    # path traversal
acbrowser --session-name "my session" open example.com # spaces
acbrowser --session-name "foo/bar" open example.com    # slashes
```
...
acbrowser --session-name secure-session open example.com
...
```bash
acbrowser open api.example.com --headers '{"Authorization": "Bearer token1"}'
acbrowser open api.acme.com --headers '{"Authorization": "Bearer token2"}'
```
...
```bash
acbrowser set headers '{"X-Custom-Header": "value"}'
```
