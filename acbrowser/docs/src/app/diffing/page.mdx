...
acbrowser diff snapshot --selector "#main" --compact
```
...
acbrowser diff screenshot --baseline before.png --output diff.png

# Adjust threshold and scope to element
acbrowser diff screenshot --baseline before.png --threshold 0.2 --selector "#hero"
```
...
acbrowser diff url https://v1.example.com https://v2.example.com --screenshot --full
```
...
```bash
acbrowser snapshot -i          # Take interactive-only snapshot (baseline)
acbrowser fill @e3 "test@example.com"
acbrowser diff snapshot        # Compare current snapshot to the baseline
```
...
```bash
# Save baseline
acbrowser open https://example.com && acbrowser snapshot > baseline.txt

# Later, check for changes
acbrowser open https://example.com && acbrowser diff snapshot --baseline baseline.txt
```
...
```bash
acbrowser open https://staging.example.com && acbrowser screenshot baseline.png
# ... deploy happens ...
acbrowser open https://staging.example.com && acbrowser diff screenshot --baseline baseline.png
```
