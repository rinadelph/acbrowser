...
Create an `acbrowser.json` file to set persistent defaults instead of repeating flags on every command.

## Config File Locations

acbrowser checks two locations, merged in priority order:
...
<table>
  ...
  <tbody>
    <tr><td>1 (lowest)</td><td><code>~/.acbrowser/config.json</code></td><td>User-level defaults</td></tr>
    <tr><td>2</td><td><code>./acbrowser.json</code></td><td>Project-level overrides</td></tr>
...
```bash
acbrowser --config ./ci-config.json open example.com
AGENT_BROWSER_CONFIG=./ci-config.json acbrowser open example.com
```
...
```bash
acbrowser --headed false open example.com
```

A bare flag is equivalent to passing `true`:

```bash
acbrowser --headed open example.com       # same as --headed true
acbrowser --headed true open example.com  # explicit
```
...
Extensions from user-level and project-level configs are **concatenated**, not replaced. For example, if `~/.acbrowser/config.json` specifies `["/ext1"]` and `./acbrowser.json` specifies `["/ext2"]`, the result is `["/ext1", "/ext2"]`.
...
- **Auto-discovered config files** (`~/.acbrowser/config.json`, `./acbrowser.json`) that are missing are silently ignored.
...
> **Tip:** If your project-level `acbrowser.json` contains environment-specific values (paths, proxies), consider adding it to `.gitignore`.
