...
Add `--observe` to any command to enable the dashboard:

```bash
agent-browser --observe open example.com
```

Then open `http://localhost:9223` in your browser to see the live dashboard.

### Custom port

```bash
agent-browser --observe 9224 open example.com
# Dashboard at http://localhost:9224
```

### Environment variable

You can also use the `AGENT_BROWSER_STREAM_PORT` environment variable directly:
