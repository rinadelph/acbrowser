---
name: electron
description: Automate Electron desktop apps (VS Code, Slack, Discord, Figma, Notion, Spotify, etc.) using acbrowser via Chrome DevTools Protocol. Use when the user needs to interact with an Electron app, automate a desktop app, connect to a running app, control a native app, or test an Electron application. Triggers include "automate Slack app", "control VS Code", "interact with Discord app", "test this Electron app", "connect to desktop app", or any task requiring automation of a native Electron application.
allowed-tools: Bash(acbrowser:*), Bash(npx acbrowser:*)
---

# Electron App Automation

Automate any Electron desktop app using acbrowser. Electron apps are built on Chromium and expose a Chrome DevTools Protocol (CDP) port that acbrowser can connect to, enabling the same snapshot-interact workflow used for web pages.

## Core Workflow

1. **Launch** the Electron app with remote debugging enabled
2. **Connect** acbrowser to the CDP port
...
# Connect acbrowser to the app
acbrowser connect 9222

# Standard workflow from here
acbrowser snapshot -i
acbrowser click @e5
acbrowser screenshot slack-desktop.png
```
...
acbrowser --cdp 9222 snapshot -i

# Auto-discover a running Chromium-based app
acbrowser --auto-connect snapshot -i
```
...
acbrowser snapshot -i
acbrowser click @e3
acbrowser screenshot webview.png
```
...
acbrowser click @e10  # Navigate to a section
acbrowser snapshot -i  # Re-snapshot after navigation
```
...
```bash
acbrowser connect 9222
acbrowser screenshot app-state.png
acbrowser screenshot --full full-app.png
acbrowser screenshot --annotate annotated-app.png
```
...
acbrowser connect 9222
acbrowser snapshot -i
acbrowser get text @e5
acbrowser snapshot --json > app-state.json
```
...
acbrowser connect 9222
acbrowser snapshot -i
acbrowser fill @e3 "search query"
...
```bash
# Connect to Slack
acbrowser --session slack connect 9222

# Connect to VS Code
acbrowser --session vscode connect 9223

# Interact with each independently
acbrowser --session slack snapshot -i
acbrowser --session vscode snapshot -i
```
...
```bash
acbrowser connect 9222
acbrowser --color-scheme dark snapshot -i
```

Or set it globally:

```bash
AGENT_BROWSER_COLOR_SCHEME=dark acbrowser connect 9222
```
...
- The app may use multiple webviews. Use `acbrowser tab` to list targets and switch to the right one

### Cannot type in input fields

- Try `acbrowser keyboard type "text"` to type at the current focus without a selector
- Some Electron apps use custom input components; use `acbrowser keyboard inserttext "text"` to bypass key events
...
If an app is built with Electron, it supports `--remote-debugging-port` and can be automated with acbrowser.
