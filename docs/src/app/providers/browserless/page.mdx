...
[Browserless](https://browserless.io) provides cloud browser infrastructure with a Sessions API. Use it when running acbrowser in environments where a local browser isn't available.

## Setup

```bash
export BROWSERLESS_API_KEY="your-api-token"
acbrowser -p browserless open https://example.com
```
...
When enabled, acbrowser connects to a Browserless cloud session instead of launching a local browser. All commands work identically.
