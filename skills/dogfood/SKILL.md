...
Always use `acbrowser` directly -- never `npx acbrowser`. The direct binary uses the fast Rust client. `npx` routes through Node.js and is significantly slower.
...
```bash
acbrowser --session {SESSION} open {TARGET_URL}
acbrowser --session {SESSION} wait --load networkidle
```
...
```bash
acbrowser --session {SESSION} snapshot -i
# Identify login form refs, fill credentials
acbrowser --session {SESSION} fill @e1 "{EMAIL}"
acbrowser --session {SESSION} fill @e2 "{PASSWORD}"
acbrowser --session {SESSION} click @e3
acbrowser --session {SESSION} wait --load networkidle
```
...
```bash
acbrowser --session {SESSION} state save {OUTPUT_DIR}/auth-state.json
```
...
```bash
acbrowser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/initial.png
acbrowser --session {SESSION} snapshot -i
```
...
```bash
acbrowser --session {SESSION} snapshot -i
acbrowser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/{page-name}.png
acbrowser --session {SESSION} errors
acbrowser --session {SESSION} console
```
...
```bash
acbrowser --session {SESSION} record start {OUTPUT_DIR}/videos/issue-{NNN}-repro.webm
```
...
```bash
acbrowser --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-1.png
sleep 1
# Perform action (click, fill, etc.)
sleep 1
acbrowser --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-2.png
sleep 1
...
```bash
sleep 2
acbrowser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}-result.png
```

4. **Stop the video:**

```bash
acbrowser --session {SESSION} record stop
```
...
```bash
acbrowser --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}.png
```
...
```bash
acbrowser --session {SESSION} close
```
...
- **Be efficient with commands.** Batch multiple `acbrowser` commands in a single shell call when they are independent (e.g., `acbrowser ... screenshot ... && acbrowser ... console`). Use `acbrowser --session {SESSION} scroll down 300` for scrolling -- do not use `key` or `evaluate` to scroll.
