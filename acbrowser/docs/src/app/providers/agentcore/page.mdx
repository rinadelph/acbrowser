...
[AWS Bedrock AgentCore](https://aws.amazon.com/bedrock/agentcore/) provides cloud browser sessions with SigV4 authentication. Use it when running acbrowser in AWS environments or when you need managed cloud browsers backed by AWS infrastructure.
...
If using SSO, run `aws sso login` before launching acbrowser. Set `AWS_PROFILE` to select a specific named profile.
...
AGENTCORE_PROFILE_ID=my-profile acbrowser -p agentcore open https://x.com/home

# With explicit region
AGENTCORE_REGION=eu-west-1 acbrowser -p agentcore open https://example.com

# With SSO profile
AWS_PROFILE=my-sso-profile acbrowser -p agentcore open https://example.com
```

When enabled, acbrowser connects to an AgentCore cloud browser session instead of launching a local browser. All commands work identically.
