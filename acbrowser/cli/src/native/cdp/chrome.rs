...
impl Default for LaunchOptions {
    fn default() -> Self {
        Self {
            ...
            stealth: true,  // Default to stealth mode (CloakBrowser)
        }
    }
}
...
fn build_chrome_args(options: &LaunchOptions) -> Result<ChromeArgs, String> {
    ...
    // Auto-add --no-sandbox on Linux (required in containers, VMs, and most environments)
    #[cfg(target_os = "linux")]
    {
...
/// Find CloakBrowser stealth Chromium (patched for bot detection bypass)
/// Checks ~/.cloakbrowser/ directory for downloaded stealth browser
fn find_cloakbrowser_chrome() -> Option<PathBuf> {
    ...
    for version in versions {
        ...
        if chrome_path.exists() {
            // Auto-add --no-sandbox on Linux if needed
            let _ = writeln!(
                std::io::stderr(),
...
pub fn find_chrome() -> Option<PathBuf> {
    // 0. Check for CloakBrowser stealth Chromium (highest priority for this fork)
    if let Some(p) = find_cloakbrowser_chrome() {
        return Some(p);
    }
