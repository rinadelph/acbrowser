...
acbrowser --cdp "ws://localhost:9222/devtools/browser/abc123" open example.com
```
...
acbrowser --auto-connect open example.com
acbrowser --auto-connect snapshot

# Or via environment variable
AGENT_BROWSER_AUTO_CONNECT=1 acbrowser snapshot
```
...
```bash
acbrowser --cdp 9222 --color-scheme dark open https://example.com
acbrowser --cdp 9222 snapshot  # stays in dark mode
```

Or set it globally via config or environment variable:

```bash
AGENT_BROWSER_COLOR_SCHEME=dark acbrowser --cdp 9222 open https://example.com
```
