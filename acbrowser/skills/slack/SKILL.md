...
acbrowser open https://app.slack.com
```
...
acbrowser click @e21  # Ref for "More unreads" button
acbrowser screenshot slack-unreads.png
```
...
acbrowser click @e14  # Activity tab (ref may vary)
acbrowser wait 1000
acbrowser screenshot activity-unreads.png

# Or check DMs tab
acbrowser click @e13  # DMs tab
acbrowser screenshot dms.png

# Or expand "More unreads" in sidebar
acbrowser click @e21  # More unreads button
acbrowser wait 500
acbrowser screenshot expanded-unreads.png
```
...
acbrowser click @e94  # Example: engineering channel ref
acbrowser wait --load networkidle
acbrowser screenshot channel.png
```
...
# Use Slack search
acbrowser snapshot -i
acbrowser click @e5  # Search button (typical ref)
acbrowser fill @e_search "keyword"
...
acbrowser snapshot --json > slack-snapshot.json
...
acbrowser snapshot -i
acbrowser screenshot channel-details.png

# Scroll through messages
acbrowser scroll down 500
acbrowser screenshot channel-messages.png
```
...
acbrowser screenshot --annotate slack-state.png

# Take full-page screenshot
acbrowser screenshot --full slack-full.png
...
acbrowser get text @e_message_ref
```
...
acbrowser snapshot --json > output.json
...
acbrowser snapshot -i | grep -c "treeitem"
# Each treeitem with a channel name in the unreads section is one unread
```

## Best Practices

- **Connect to existing sessions**: Use `acbrowser connect 9222` if Slack is already open. This is faster than opening a new browser.
...
- **Scroll in sidebar**: Use `acbrowser scroll down 300 --selector ".p-sidebar"` to scroll within the Slack sidebar if channel list is long.
...
acbrowser get url
acbrowser get title
acbrowser screenshot page-state.png
```
...
acbrowser snapshot -i > snapshot.txt

# Check Activity tab for unreads
acbrowser click @e14  # Activity tab
acbrowser wait 1000
acbrowser screenshot activity.png
ACTIVITY_RESULT=$(acbrowser get text @e_main_area)
...
acbrowser click @e21  # More unreads button
acbrowser wait 500
acbrowser snapshot -i > unreads-expanded.txt
acbrowser screenshot unreads.png
