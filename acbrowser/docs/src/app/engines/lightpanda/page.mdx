...
acbrowser manages Lightpanda the same way it manages Chrome -- spawning the process, connecting via CDP, and shutting it down. All downstream commands (snapshot, click, fill, screenshot, etc.) work through the same CDP protocol path.

## Installation

Install the Lightpanda binary before using it with acbrowser:
...
acbrowser --engine lightpanda snapshot
acbrowser --engine lightpanda screenshot
```
...
Or in your `acbrowser.json` config:
...
acbrowser returns a clear error if you combine `--engine lightpanda` with unsupported flags.
