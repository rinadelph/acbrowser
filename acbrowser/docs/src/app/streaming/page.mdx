...
```bash
AGENT_BROWSER_STREAM_PORT=9223 acbrowser open example.com
```

You can also manage streaming at runtime:

```bash
acbrowser stream status            # Show streaming state and bound port
acbrowser stream enable --port 9223  # Re-enable on a specific port
acbrowser stream disable           # Stop streaming for the session
```
...
`acbrowser stream status --json` returns data like:
...
```typescript
import { BrowserManager } from 'acbrowser';
