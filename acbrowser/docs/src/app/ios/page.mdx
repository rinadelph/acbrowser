...
acbrowser -p ios swipe down
acbrowser -p ios swipe left
acbrowser -p ios swipe right

# Swipe with distance (pixels)
acbrowser -p ios swipe up 500
...
### 3. Use with acbrowser

```bash
# Connect device via USB, then use the UDID
acbrowser -p ios --device "<DEVICE_UDID>" open https://example.com

# Or use the device name if unique
acbrowser -p ios --device "John's iPhone" open https://example.com
```
...
ensure it works, then retry with acbrowser.
